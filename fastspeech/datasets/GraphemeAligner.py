...
class Segment:
    ...
    def __repr__(self):
        return f"{self.label}\t({self.score:4.2f}): [{self.start:5d}, " \
               f"{self.end:5d}) "
...
def _backtrack(trellis, emission, tokens, blank_id=0):
    ...
    for t in range(t_start, 0, -1):
        # 1. Figure out if the current position was stay or change Note (
        # again): `emission[J-1]` is the emission at time frame `J` of
        # trellis dimension. Score for token staying the same from time
        # frame J-1 to T.
...
class GraphemeAligner(nn.Module):
    ...
    def forward(
        ...
        for index in range(batch_size):
            ...
            segments = _merge_repeats(texts[index], path)
